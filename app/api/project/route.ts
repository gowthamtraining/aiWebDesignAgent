...
import { Forge_CHAT_PROMPT, Forge_INTENT_PROMPT, WEB_ANALYSIS_PROMPT, WEB_GENERATION_PROMPT } from "@/lib/prompt";
...
export async function GET() {
    try {
        ...
        if (error) {
            console.error("[GET /api/project] Database error:", error);
...
- Context from previous pages : ${previousPagesContext}
    ...
    const summaryResult = await insforge.ai.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: `You are Forge, an AI web design agent. You just finished building pages.

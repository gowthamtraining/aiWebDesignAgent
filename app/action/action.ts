...
export const generateProjectTitle = async (message: string) => {
    ...
    - Return ONLY the name, nothing else.`,
        ...
        console.log("[generateProjectTitle] AI result success");
        const text = result.choices?.[0]?.message?.content || "";
        return text.trim() || "Untitled Project"
    } catch (error) {
        console.error("[generateProjectTitle] error:", error);
...
export const convertModelMessages = async (messages: UIMessage[]) => {
    const modelMessages = messages.map((message: UIMessage) => {
        ...
        const content = contentParts.length === 1 && contentParts?.[0].type === "text" ? contentParts[0].part : contentParts;
